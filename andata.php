...
include "connection.php";
    $statement = $conn->prepare ("SELECT ID, LINEA, DATE_FORMAT(`GARLASCO_vittoria`, '%H.%i') AS `GARLASCO_vittoria`, DATE_FORMAT(`GARLASCO_pavia`, '%H.%i') AS `GARLASCO_pavia`, DATE_FORMAT(`GROPELLO`, '%H.%i') AS `GROPELLO`, DATE_FORMAT(`FAMAGOSTA`, '%H.%i') AS `FAMAGOSTA`  FROM `andata` ORDER by GARLASCO_vittoria asc");
    $statement->execute( );
